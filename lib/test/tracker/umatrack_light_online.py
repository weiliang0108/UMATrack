...
import cv2
import os
from lib.models.fusiontrans_light import build_fusiontrans_light_online_score
...
class MixFormerOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        network = build_fusiontrans_light_online_score(params.cfg, train=False)
    ...
    def track(self, image, info: dict = None):
        ...
        if self.frame_id % self.update_interval == 0:
            ...
            if self.online_size > 1:
                with torch.no_grad():
                    # self.network.set_online(self.template, self.online_template)
                    self.network.set_online(self.template, self.online_max_template)
