...
def trackerlist(name: str, parameter_name: str, dataset_name: str, run_ids = None, display_name: str = None,
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, dataset_name, run_id, display_name, result_only, tracker_params) for run_id in run_ids]
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, display_name: str = None,
        ...
        if self.run_id is None:
            # if 'Online' in tracker_params['model']:

            self.results_dir = '{}/{}/{}_{}/{}'.format(env.results_path, self.name, self.parameter_name, self.dataset_name, tracker_params['model'][23:27])
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # time_net = np.zeros(len(seq.frames[1:]))
        # time_net[0] = time.time() - start_time
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            start_time = time.time()
            out = tracker.track(image, info)
            # time_net[frame_num] = time.time() - start_time
            prev_output = OrderedDict(out)
