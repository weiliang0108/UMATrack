...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    # print('FPS2: {}'.format(num_frames / time_net))

    # if isinstance(output['time_net'][0], (dict, OrderedDict)):
    #     exec_time_net = sum([sum(times.values()) for times in output['time_net']])
    #     num_frames_net = len(output['time_net'])
    # else:
    #     exec_time_net = sum(output['time_net'])
    #     num_frames_net = len(output['time_net'])
    #
    # print('FPS_net: {}'.format(num_frames_net / exec_time_net))
