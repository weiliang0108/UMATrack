...
def transform_got10k(tracker_name, cfg_name, ep):
    env = env_settings()
    result_dir = env.results_path
    src_dir = os.path.join(result_dir, "%s/%s/%s/got10k/" % (tracker_name, cfg_name, ep))
    dest_dir = os.path.join(result_dir, "%s/%s/%s/got10k_submit/" % (tracker_name, cfg_name, ep))
...
if __name__ == "__main__":
    ...
    args.cfg_name = 'baseline_got10k_test'

    ep = '00ET'
    transform_got10k(args.tracker_name, args.cfg_name, ep)
