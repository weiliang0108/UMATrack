    ...
    for name in name_list:
        ...
        if name == "GOT10K_votval":
            if settings.use_lmdb:
                ...
                datasets.append(Got10k(settings.env.got10k_dir, image_loader=image_loader))
        if name == "COCO17":
            if settings.use_lmdb:
...
def build_dataloaders(cfg, settings):
    ...
    data_processing_train = processing.UMATrackProcessing(search_area_factor=search_area_factor,
                                                           output_sz=output_sz,
    ...
    data_processing_val = processing.UMATrackProcessing(search_area_factor=search_area_factor,
                                                         output_sz=output_sz,
                                                         center_jitter_factor=settings.center_jitter_factor,
                                                         scale_jitter_factor=settings.scale_jitter_factor,
                                                         mode='sequence',
