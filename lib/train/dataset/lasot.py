...
class Lasot(BaseVideoDataset):
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        if split is not None:
            ...
            sequence_list = pandas.read_csv(file_path, header=None).squeeze("columns").values.tolist() # for pandas>1.5
    ...
    def _get_class(self, seq_path):
        normalized_path = seq_path.replace('\\', '/')
        raw_class = normalized_path.split('/')[-2]
        return raw_class
