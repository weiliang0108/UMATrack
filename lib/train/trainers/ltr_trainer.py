...
class LTRTrainer(BaseTrainer):
    ...
    def _print_stats(self, i, loader, batch_size):
        ...
        if i % self.settings.print_interval == 0 or i == loader.__len__():
            epoch_resttime = (loader.__len__() - i) * ((current_time - self.prev_time2) / self.settings.print_interval)
            total_resttime = (loader.__len__()) * ((current_time - self.prev_time2) / self.settings.print_interval) * (500 - self.epoch) + (
                        loader.__len__() - i) * ((current_time - self.prev_time2) / self.settings.print_interval)

            epoch_resttime = datetime.timedelta(seconds=epoch_resttime)
            total_resttime = datetime.timedelta(seconds=total_resttime)
            ...
            print_str += '[epoch_resttime: %s, total_resttime : %s] ' % (epoch_resttime, total_resttime)
