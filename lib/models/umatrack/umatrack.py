...
from lib.models.umatrack.head import build_box_head
    ...
    if config.MODEL.BACKBONE.PRETRAINED and train:
        ...
        for k, v in ckpt.items():
            if 'pos_embed' not in k and 'mask_token' not in k:
                new_dict[k] = v
        missing_keys, unexpected_keys = ef.load_state_dict(new_dict, strict=False)
        if is_main_process():
...
class UMATrack(nn.Module):
    ...
    def forward(self, template, online_template, search, run_score_head=False, gt_bboxes=None):
        ...
        search = self.backbone(template, online_template, search)
...
def build_umatrack(cfg, train=True) -> UMATrack:
    backbone = get_ef_mhm(cfg, train)  # backbone without positional encoding and attention mask
