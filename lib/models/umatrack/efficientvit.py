...
class PatchMerging(torch.nn.Module):
    def __init__(self, dim, out_dim, input_resolution):
        ...
        self.conv2 = Conv2d_BN(hid_dim, hid_dim, 3,1, 1, groups=hid_dim, resolution=input_resolution)
...
class Residual(torch.nn.Module):
    ...
    def forward(self, x):
        if self.training and self.drop > 0:
            ...
            return x + self.m(x)  # x=[2,64,16,16]
...
class EfficientViT(torch.nn.Module):
    def __init__(self, img_size=224,
        ...
        resolution = 256 // patch_size
        attn_ratio = [embed_dim[i] / (key_dim[i] * num_heads[i]) for i in range(len(embed_dim))]
        self.blocks1 = []
        self.blocks2 = []
        self.blocks3 = []

        # Build EfficientViT blocks， i是stage索引，三个stage， 每个stage有dpth个EfficientViTBlock
    ...
    def forward(self, x):
        ...
        x2 = self.blocks1(x1)  # x2=[2,64,16,16]
        x3 = self.blocks2(x2)  # x3=[2,128,8,8]
        # x4 = self.blocks3(x3)  # x4=[2,192,4,4]
        # x5 = torch.nn.functional.adaptive_avg_pool2d(x4, 1).flatten(1)
...
def EfficientViT_M4():
    patch_size = 16
    model = EfficientViT(
        patch_size=patch_size, num_classes=256,
        embed_dim=[128, 256, 384], depth=[1, 2, 3], num_heads=[4, 4, 4],window_size=[7, 7, 7],
                 kernels=[7, 5, 3, 3]
    )
    # pretrained = None
    ...
    # if pretrained:
        ...
        # print(f'Loaded checkpoint from local: {pretrained_path}')
        # d = checkpoint['model']
        # D = model.state_dict()
        # p=[]
        # q=[]
        ...
        # load_matching_weights(model, d)
        # print('Lord checkpoint', pretrained)

    return model

if __name__ == '__main__':
    x = torch.FloatTensor(2,3,256,256)
    net = EfficientViT()
    print(x.shape)
    net(x)
